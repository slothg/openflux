		...
		public function update(indices:Array = null):void {
			if (container is PlexiContainer) {
				...
				for each (child in container.children) {
					if (!watchers[child]) {
						view.planes[child].x = child.width / 2 + child.x;
						view.planes[child].y = (child.height / 2 + child.y) * -1;
						watchers[child] = [];
						...
						for each (p in props) {
							watchers[child].push(BindingUtils.bindProperty(view.planes[child], p, child, p));
						}
					}
				}
			}
